...
const connectToDB = require("./dataConnection");
...
var carts = [];
var orders = [];
var uploadfolder = path.join(__dirname, "root", "images");
/////////////////////////////
connect();
getCategoriesDB();
getProductsDB();
app.get("/", async (req, res) => {
  await getCategoriesDB();
  await getProductsDB();
...
app.get("/products", async (req, res) => {
  if (!products.length) await getProductsDB();
  if (!categories.length) await getCategoriesDB();
...
app.post("/save-product", async (req, res) => {
  ...
  var prodModel = new dataModel.productModel(data);
